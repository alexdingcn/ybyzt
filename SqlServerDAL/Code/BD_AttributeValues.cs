...
namespace Hi.SQLServerDAL
{
    ...
    public partial class BD_AttributeValues
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete [BD_AttributeValues] ");
            strSql.Append(" where [ID]=@ID");
            SqlParameter[] parameters = {
                    new SqlParameter("@ID", SqlDbType.Int)};
        ...
        public int GetMaxId()
        {
            return SqlHelper.GetMaxID(SqlHelper.LocalSqlServer, "[ID]", "[BD_AttributeValues]");
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [BD_AttributeValues]");
            strSql.Append(" where [ID]= @ID");
            SqlParameter[] parameters = {
                    new SqlParameter("@ID", SqlDbType.Int)};
            parameters[0].Value = ID;
        ...
        /// </summary>
        public IList<Hi.Model.BD_AttributeValues> GetList(string strSql)
        {
        ...
        /// 获取泛型数据列表,在单表查询时使用
        /// </summary>
        public IList<Hi.Model.BD_AttributeValues> GetList(string strWhat, string strWhere, string strOrderby)
        {
        ...
        /// 分页获取泛型数据列表
        /// </summary>
        public IList<Hi.Model.BD_AttributeValues> GetList(int pageSize, int pageIndex, string fldSort, bool sort, string strCondition, out int pageCount, out int count)
        {
            string strSql;
            DataSet ds = SqlHelper.PageList(SqlHelper.LocalSqlServer, "[BD_AttributeValues]", null, pageSize, pageIndex, fldSort, sort, strCondition, "ID", false, out pageCount, out count, out strSql);
            return GetList(ds);
        }
        #endregion
        ...
        /// 由一行数据得到一个实体
        /// </summary>
        private Hi.Model.BD_AttributeValues GetModel(DataRow r)
        {
            ...
            model.AttrCode = SqlHelper.GetString(r["AttrCode"]);
            model.AttrValue = SqlHelper.GetString(r["AttrValue"]);
        ...
        /// 由数据集得到泛型数据列表
        /// </summary>
        private IList<Hi.Model.BD_AttributeValues> GetList(DataSet ds)
        {
            return Common.GetListEntity<Hi.Model.BD_AttributeValues>(ds.Tables[0]);
        }
    }
}

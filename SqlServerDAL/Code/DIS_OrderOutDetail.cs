...
namespace Hi.SQLServerDAL
{
    ...
    public partial class DIS_OrderOutDetail
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Hi.Model.DIS_OrderOutDetail model)
        {
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Hi.Model.DIS_OrderOutDetail model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete [DIS_OrderOutDetail] ");
            strSql.Append(" where [ID]=@ID");
            SqlParameter[] parameters = {
                    new SqlParameter("@ID", SqlDbType.BigInt)};
        ...
        public int GetMaxId()
        {
            return SqlHelper.GetMaxID(SqlHelper.LocalSqlServer, "[ID]", "[DIS_OrderOutDetail]");
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [DIS_OrderOutDetail]");
            strSql.Append(" where [ID]= @ID");
            SqlParameter[] parameters = {
                    new SqlParameter("@ID", SqlDbType.BigInt)};
            parameters[0].Value = ID;
        ...
        /// 得到一个对象实体
        /// </summary>
        public Hi.Model.DIS_OrderOutDetail GetModel(int ID)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from [DIS_OrderOutDetail] ");
            strSql.Append(" where [ID]=@ID");
            SqlParameter[] parameters = {
                    new SqlParameter("@ID", SqlDbType.BigInt)};
            parameters[0].Value = ID;
        ...
        /// </summary>
        public IList<Hi.Model.DIS_OrderOutDetail> GetList(string strSql)
        {
        ...
        /// 获取泛型数据列表,在单表查询时使用
        /// </summary>
        public IList<Hi.Model.DIS_OrderOutDetail> GetList(string strWhat, string strWhere, string strOrderby)
        {
        ...
        /// 分页获取泛型数据列表
        /// </summary>
        public IList<Hi.Model.DIS_OrderOutDetail> GetList(int pageSize, int pageIndex, string fldSort, bool sort, string strCondition, out int pageCount, out int count)
        {
            string strSql;
            DataSet ds = SqlHelper.PageList(SqlHelper.LocalSqlServer, "[DIS_OrderOutDetail]", null, pageSize, pageIndex, fldSort, sort, strCondition, "ID", false, out pageCount, out count, out strSql);
            return GetList(ds);
        }
        #endregion
        ...
        {
            return Common.GetListEntity<Hi.Model.DIS_OrderOutDetail>(ds.Tables[0]);
        }
    }
}

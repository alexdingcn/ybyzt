...
namespace Hi.SQLServerDAL
{
    ...
    public partial class BD_DisPrice
    {
        ...
        public int GetMaxId()
        {
            return SqlHelper.GetMaxID(SqlHelper.LocalSqlServer, "[ID]", "[BD_DisPrice]");
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [BD_DisPrice]");
            strSql.Append(" where [ID]= @ID");
            SqlParameter[] parameters = {
                    new SqlParameter("@ID", SqlDbType.Int)};
            parameters[0].Value = ID;
        ...
        /// 得到一个对象实体
        /// </summary>
        public Hi.Model.BD_DisPrice GetModel(int ID)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// </summary>
        public IList<Hi.Model.BD_DisPrice> GetList(string strSql)
        {
        ...
        /// 获取泛型数据列表,在单表查询时使用
        /// </summary>
        public IList<Hi.Model.BD_DisPrice> GetList(string strWhat, string strWhere, string strOrderby)
        {
        ...
        /// 分页获取泛型数据列表
        /// </summary>
        public IList<Hi.Model.BD_DisPrice> GetList(int pageSize, int pageIndex, string fldSort, bool sort, string strCondition, out int pageCount, out int count)
        {
            string strSql;
            DataSet ds = SqlHelper.PageList(SqlHelper.LocalSqlServer, "[BD_DisPrice]", null, pageSize, pageIndex, fldSort, sort, strCondition, "ID", false, out pageCount, out count, out strSql);
            return GetList(ds);
        }
        #endregion
        ...
        /// 由一行数据得到一个实体
        /// </summary>
        private Hi.Model.BD_DisPrice GetModel(DataRow r)
        {
            Hi.Model.BD_DisPrice model = new Hi.Model.BD_DisPrice();
            model.ID = SqlHelper.GetInt(r["ID"]);
            model.CompID = SqlHelper.GetInt(r["CompID"]);
            model.Type = SqlHelper.GetInt(r["Type"]);
            model.One = SqlHelper.GetInt(r["One"]);
            model.Two = SqlHelper.GetInt(r["Two"]);
            model.Three = SqlHelper.GetInt(r["Three"]);
            model.IsEnabled = SqlHelper.GetBool(r["IsEnabled"]);
            model.DisIDs = SqlHelper.GetString(r["DisIDs"]);
        ...
        /// 由数据集得到泛型数据列表
        /// </summary>
        private IList<Hi.Model.BD_DisPrice> GetList(DataSet ds)
        {
            return Common.GetListEntity<Hi.Model.BD_DisPrice>(ds.Tables[0]);
        }
    }
}

...
================================================<br> 
 ...
 ,SYS_PaymentBank.bankAddress,SYS_PaymentBank.bankprivate,SYS_PaymentBank.bankcity", "SYS_PaymentBank", "  SYS_PaymentBank.Isno=1  and SYS_PaymentBank.dr=0");
    ...
    [WebMethod(Description = "入金记录状态修改")]
    public string UpPAY_FC_State(string json)
    {
        ...
        try
        {
            ...
            if (Jdata != null)
            {
                string guid = Jdata["guid"].ToString();
                List<Hi.Model.PAY_Financing> PF = new Hi.BLL.PAY_Financing().GetList("", " isnull(dr,0)=0 and guid='" + guid + "'", "");
                if (PF.Count > 0)
                {
                    PF[0].State = 1;
                    PF[0].ts = DateTime.Now;
                    if (new Hi.BLL.PAY_Financing().Update(PF[0]))
                    {
                        Msg.result = true;
                        Msg.code = "修改成功";
                    }
        ...
        {
            Msg.result = false;
            Msg.code = "修改失败，" + ex.Message + "";
        }
        return new JavaScriptSerializer().Serialize(Msg);
    }


    [WebMethod(Description = "在线融资记录状态修改")]
    public string UpPAY_FC_State_1(string json)
    {
        Common.ResultMessage Msg = new Common.ResultMessage();
        try
        {
            ...
            if (Jdata != null)
            {
                ...
                string loan_loanno = Jdata["loan_loanno"].ToString();
                decimal price = decimal.Parse(Jdata["price"].ToString()) / 100;
                List<Hi.Model.PAY_Financing> PF = new Hi.BLL.PAY_Financing().GetList("", " isnull(dr,0)=0 and vdef2='" + loan_loanno + "'", "");
                if (state == "1")
                {
                    if (PF.Count > 0)
                    {
                    ...
                    {
                        Msg.result = false;
                        Msg.code = "修改失败，数据不存在(loan_loanno：" + loan_loanno + ")";
                    }
                }
                else
                {
    ...
    [WebMethod(Description = "基础档案同步/获取订单")]
    public string SYJInsert(string Xml)
    {
        return new ImportDisProD().TransFerBasics(Xml);
    }

    [WebMethod(Description = "NC获取订单")]
    public string NCGetOrder(string compID, string startDate = null, string endDate = null)
    {
        Type type = typeof(NCLoadIN);
        object obj = Activator.CreateInstance(type);
        System.Reflection.MethodInfo method = type.GetMethod("TransNCOrder");
        object[] str = { compID, startDate, endDate };
        string resulet = (string)method.Invoke(obj, str);
        return resulet;
    }

    [WebMethod(Description = "InsertCusClass:代理商分类， InsertCus：代理商")]
    public string NCDisInsert(string XML)
    {
        return new NCLoadIN().TransFerBasics(XML);
    }

...
public partial class EShopLogReport : System.Web.UI.Page
{
    ...
    {
        string sql = string.Format("select top 30 COUNT(*) as num,Module from (select * from A_SystemLog where Module like '企业店铺：%' {0} ) a group by Module order by num desc", sqlwhere);
        return sql;
    }

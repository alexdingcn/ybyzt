...
public partial class Admin_Role_RoleEdit : AdminPageBase
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (KeyID != 0)
            {
                ...
                {
                    string sql = "select * from SYS_AdminUser  where ISNULL(dr,0)=0 and IsEnabled=1 and RoleID=" + KeyID;
                    ...
                    if (userDT.Rows.Count > 0)
                    {
                        ...
                        {
                            JScript.AlertMethod(this, "请先禁用该岗位下的人员！", JScript.IconOption.错误, "function (){ location.replace('" + ("RoleInfo.aspx?KeyID=" + KeyID) + "'); }");
                            return;
                        }
                    }
                }
                ...
                if (new Hi.BLL.SYS_Role().Update(role))
                {
                    ...
                    else
                    {
                       ...
                       // Response.Redirect("RoleInfo.aspx?KeyID=" + KeyID);
                    }
                }
                else
                {
    ...
    public bool RoleExistsAttribute(string name, string value, string id = "")
    {
        ...
        {
            List<Hi.Model.SYS_Role> Dis = new Hi.BLL.SYS_Role().GetList("", " " + name + "='" + value + "' and isnull(DisID,0)=0 and CompID=" + this.UserID + " and id<>'" + id + "' and isnull(dr,0)=0 ", "");
            if (Dis.Count > 0)
            {
                exists = true;
            }
        ...
        else
        {
            ...
            List<Hi.Model.SYS_Role> Dis = new Hi.BLL.SYS_Role().GetList("", " " + name + "='" + value + "' and isnull(DisID,0)=0 and CompID=" + this.UserID + " and isnull(dr,0)=0 ", "");
            if (Dis.Count > 0)
            {
                exists = true;
            }

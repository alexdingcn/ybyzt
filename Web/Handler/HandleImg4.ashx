<%@ WebHandler Language="C#" Class="HandleImg4" %>
...
public class HandleImg4 : System.Web.SessionState.IRequiresSessionState, IHttpHandler
{
        ...
        if (context.Request.HttpMethod == "POST")
        {
           ...
           if (logUser != null)
           {
               try
               {
                   ...
                   string filename = context.Request["upLoadImg"].ToString();
                   ...
                   var toFileName = name;
                   string saveFile = Common.GetWebConfigKey("ImgPath") + "PicSpace/"+logUser.CompID + filename; //path + "D" + toFileName;
                   //var HttpFile = context.Request.Files["upLoadImg"];
                   //HttpFile.SaveAs(saveFile);
                   string ThumbPath = path + "D" + toFileName;
                   System.Drawing.Image pic = System.Drawing.Image.FromFile(saveFile);//strFilePath是该图片的绝对路径
                   int intWidth = pic.Width;//长度像素值
                   int intHeight = pic.Height;//高度像素值 
                   MakeThumbnail(saveFile, ThumbPath, intWidth, intHeight, "Cut2");
                   //大缩略图
                   ...
                   string avatar_path = viewPath + toFileName; //HandleHeadImg(path, name, b);

...
public partial class Company_Financing_SettlementAdd : CompPageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            List<Hi.Model.PAY_Withdrawals> withdrawals = new Hi.BLL.PAY_Withdrawals().GetList("", "isnull(dr,0)=0  and DisID=0 and CompID=" + CompID, "");
            ...
            {
                List<Hi.Model.PAY_OpenAccount> openAcc = new Hi.BLL.PAY_OpenAccount().GetList("", " isnull(dr,0)=0 and state=1 and DisID=0 and CompID=" + CompID, "");
                if (openAcc.Count > 0)
                {
    ...
    protected void Btn_Save(object sender, EventArgs e)
    {
        ...
        List<Hi.Model.PAY_Withdrawals> Lwithdrawals = new Hi.BLL.PAY_Withdrawals().GetList("", " isnull(dr,0)=0  and DisID=0 and CompID=" + CompID, "");
        ...
        if (Lwithdrawals.Count > 0)
        {
            try
            {
                ...
                Lwithdrawals[0].modifyuser = UserID;
                //结算账户接口参数赋值
        ...
        else
        {
            try
            {
                ...
                withdrawals.DisID = openAcc[0].DisID;
                withdrawals.CompID = CompID;
                withdrawals.State = 2;

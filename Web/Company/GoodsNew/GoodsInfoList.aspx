<%@ Page Language="C#" AutoEventWireup="true" CodeFile="GoodsInfoList.aspx.cs" Inherits="Company_Goods_GoodsInfoList" %>
    ...
    <div class="rightinfo">
        ...
        <div class="place">
            <ul class="placeul">
                ...
                <li>></li>
                <li><a href="../GoodsNew/GoodsInfoList.aspx" runat="server" id="atitle">商品库存</a></li>
            </ul>
        </div>
        <!--当前位置 end-->
        <!--功能按钮 start-->
        <div class="tools">
            <ul class="toolbar left" style="margin-bottom: 5px">
                ...
                <li onclick="addList()" style="display: none;"><span>
                    <img src="../images/t14.png" /></span>Excel导入</li>
                <li id="libtnNext" runat="server" style="display: none;"><a href="javascript:void(0);"
                    onclick="onlinkOrder('../Pay/PayAccountList.aspx?nextstep=1','ktbdskzh')"><span>
        ...
        <table class="tablelist" id="TbList">
            <thead>
                <tr>
                    ...
                    <th style='<%= OrderInfoType.rdoOrderAudit("商品是否启用库存",this.CompID)=="0"?"": "display:none" %>'>
                        总库存
                    </th>
                    <th style='<%= OrderInfoType.rdoOrderAudit("商品是否启用库存",this.CompID)=="0"?"": "display:none" %>'>
                        可用库存
                    ...
                    <th align="center" class="t8" style="display:none">
                        操作
                    </th>
                </tr>
            </thead>
                    ...
                    <ItemTemplate>
                        <tr>
                            <td align="center">
                                ...
                                <asp:HiddenField ID="HF_GoodsId" runat="server" Value='<%# Eval("GoodsID") %>' />
                                <asp:HiddenField ID="HF_GoodsName" runat="server" Value='<%# Common.GetGoodsName(Eval("GoodsID").ToString())%>' />
                            </td>
                            <td>
                                <div class="tcle">
                                    <a href="GoodsInfo.aspx?rtype=1&nextstep=<%=Request["nextstep"] %>&goodsId=<%# Common.DesEncrypt(Eval("GoodsID").ToString(), Common.EncryptKey)%>&goodsInfoId=<%# Eval("ID") %>" class="link" style="text-decoration: underline;" tip='<%# Eval("ID") %>'
                                        title='<%# Common.GetGoodsName(Eval("GoodsID").ToString())%>' >
                                        <%# Common.GetGoodsName(Eval("GoodsID").ToString())%></a></div>
                            </td>
                            
                            <td>
                                <div class="tc">
                                    <%# Eval("ValueInfo").ToString()%></div>
                            </td>
                            <td>
                                <div class="tc">
                                    <%# GetUnti(Eval("GoodsID").ToString())%></div>
                            </td>
                            <td style='<%= OrderInfoType.rdoOrderAudit("商品是否启用库存",this.CompID)=="0"?"": "display:none" %>'>
                                <div class="tc" style="text-align: right;">
                                        <%# Eval("StockTotalNum", "{0:F2}")%>
                                    </div>
                            </td>
                            <td style='<%= OrderInfoType.rdoOrderAudit("商品是否启用库存",this.CompID)=="0"?"": "display:none" %>'>
                                <div class="tc" style="text-align: right;">
                                    <label style="" id="lblInventory" class="lblInventory">
                                        <%# Eval("Inventory","{0:F2}")%></label>
                                    <%--<input type="text" id="txtInventory" runat="server" class="textBox txtInventory"
                                        style="width: 75px; display: none" onkeyup="KeyInt3(this);" maxlength="11" />--%>
                                    <%--<a href="javascript:;" style="color: #dddddd" class="aedit2" tip="<%# Eval("ID") %>">
                                        修改</a>--%></div>
                            </td>
                            <td>
                                <div class="tc">
                            ...
                            <td>
                                <div class="tc">
                                    <%#  Eval("validDate","{0:yyyy-MM-dd}")%></div>
                            </td>
                            <td style='<%= OrderInfoType.rdoOrderAudit("商品是否启用库存",this.CompID)=="0"?"": "display:none" %>'>
                                <div class="tc" style="text-align: right;">
                                        <%# Eval("StockNum", "{0:F2}")%>
                            ...
                            <td>
                                <div class="tc">
                                    <%# Eval("isOffline").ToString()=="1"?"上架":"下架" %></div>
                            </td>
                            <td style="display:none">
                                <div class="tc">
                                    <a href="javascript:;" class="aedit" tip="<%# Eval("ID") %>">编辑</a></div>
                            </td>
                        </tr>
                    </ItemTemplate>
                </asp:Repeater>
      ...
      var div = $('<div   Msg="True"  style="z-index:10; background-color:#000; opacity:0.3; filter:alpha(opacity=50);position: fixed;width:100%;height:100%;top:0px;left:0px;"></div>');
        ...
        $(function () {
            ...
            $("#export").click(function () {
                ...
                window.location.href = '../../../ExportExcel.aspx?intype=6&searchValue='+str+'&p=' + $("#txtPageSize").val() + '&c=<%=Pager.CurrentPageIndex %>';
            });

            //修改库存
             $(document).on("click",".aedit2",function(){
                if($.trim($(this).text())=="修改"){
                    $(this).prevAll(".lblInventory").hide();
                    $(this).prevAll(".txtInventory").show();
                    $(this).prevAll(".txtInventory").val($.trim($(this).prevAll(".lblInventory").text()));
                    $(this).text("保存");
                }else{
                    var inventory=$.trim($(this).prevAll(".txtInventory").val());
                    if(inventory==""){
                        layerCommon.msg("库存不能为空", IconOption.错误);
                        return false;
                    }
                    var thiss=this;
                     $.ajax({
                         type:"post",
                         data:{ck:Math.random(),action:"UpdateInventory",Inventory:inventory,id:$(this).attr("tip")},
                         dataType:"text",
                         async:false,
                         success:function(data){
                                if(data!=""){
                                    $(thiss).prevAll(".lblInventory").text( parseFloat(inventory).toFixed(2));
                                }else{
                                    layerCommon.msg("修改库存失败了", IconOption.错误);
                                }
                         }
                     })
                    $(this).prevAll(".lblInventory").show();
                    $(this).prevAll(".txtInventory").hide();
                    $(this).text("修改");
                }
            })
            ...
            //关闭导入层
             $("input.cancel,.tiptop a").on("click", function () {
               ...
               //详细商品
        function InfoData(id,infoId) {
            location.href = "GoodsInfo.aspx?rtype=1&nextstep=<%=Request["nextstep"] %>&goodsId=" + id+"&goodsInfoId=" + infoId;
        }
        //新增商品信息
        function addGoods() {
            if ('<%=Request["nextstep"] %>' == "1") {

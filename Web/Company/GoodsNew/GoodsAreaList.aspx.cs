...
public partial class Company_GoodsNew_GoodsAreaList : CompPageBase
{
    ...
    private string Where()
    {
        ...
        string areaName =Common.NoHTML( this.txtDisName.Value.Trim());//代理商名称
        ...
        string disnamelist = string.Empty;
        ...
        {
            List<Hi.Model.BD_Distributor> l = new Hi.BLL.BD_Distributor().GetList("", "isnull(dr,0)=0 and compId=" + this.CompID + " and disname like '%" + areaName + "%'", "");
            if (l.Count > 0)
            {
                foreach (Hi.Model.BD_Distributor item in l)
                {
                    disnamelist += item.ID + ",";
                }
            }
            ...
            {
                disnamelist = disnamelist.Substring(0, disnamelist.Length - 1);
                strWhere += " and disId in(" + disnamelist + ")";
            }
        }

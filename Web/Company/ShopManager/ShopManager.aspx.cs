...
public partial class Company_ShopManager_ShopManager : CompPageBase
{
    ...
    public string GetBannerTopImg(string type, int TypeId)
    {
        ...
        List<Hi.Model.BD_ShopImageList> ShopList = ShopList = new Hi.BLL.BD_ShopImageList().GetList("GoodsUrl,ImageUrl", " dr=0 and Compid=" + CompID + "", "id");
        if (ShopList.Count == 0)
        {
            return "";
        }
    ...
    private void BindTopGoods()
    {
        ...
        List<Hi.Model.BD_ShopGoodsList> l = new Hi.BLL.BD_ShopGoodsList().GetList("Title,GoodsID,ShowName", "isnull(dr,0)=0 and compId=" + CompID, "title ");
        if (l.Count > 0)
        {
            string title = "";

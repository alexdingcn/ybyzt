...
public partial class Company_ShopManager_RecommendGoodsList : CompPageBase
{
    public int comPid = 0;
    ...
    private void Bind2()
    {
        ...
        List<Hi.Model.BD_ShopGoodsList> l = new Hi.BLL.BD_ShopGoodsList().GetList("", "isnull(dr,0)=0 and compId=" + this.CompID, "title");
        if (l.Count > 0)
        {
            string title = "";
        ...
        try
        {
            ...
            if (goodsId != null)
            {
                ...
                if (!Util.IsEmpty(this.hidtitle.Value.Trim()))
                {
                    ...
                    List<Hi.Model.BD_ShopGoodsList> ll = new Hi.BLL.BD_ShopGoodsList().GetList("", "isnull(dr,0)=0 and compId=" + this.CompID + " and title='" + title + "'", "", Tran);
                    if (ll.Count > 0)
                    {
                        new Hi.BLL.BD_ShopGoodsList().Delete(this.CompID, title, Tran);
                    }
                }
    ...
    {
        StringBuilder html = new StringBuilder();
        List<Hi.Model.BD_ShopGoodsList> l = new Hi.BLL.BD_ShopGoodsList().GetList("", "isnull(dr,0)=0 and compId=" + this.CompID + " and title='" + Common.NoHTML(title) + "'", "");
        if (l.Count > 0)
        {
            for (int i = 0; i < l.Count; i++)
            {

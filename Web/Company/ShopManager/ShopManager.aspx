    ...
    <script  type="text/javascript">
                   ...
                   var EditShopVoid = {
                       ...
                       BindBannerEvent: function () {
                           $("input:file", Showoption.btnBanner).unbind().on("change", function (e, data) {
                               var btnFile = this, str = $(btnFile).val(), Suffix = $.trim(str.substring(str.lastIndexOf("."))).toLowerCase(), $Li = $(btnFile).closest("li"), BanerLength = $(e.delegateTarget).closest(".ShowSucessBtn").prev().find("li").length;
                               ...
                               $.ajaxFileUpload({
                                   type: 'post',
                                   url: "/Controller/CompImg.ashx?FlileID=" + btnFile.id + "&Fileype=Banner",
                                   secureuri: false,
                       ...
                       BindRecommendEvent: function () {
                           ...
                           $(".adMenu-height .show dd  .SelectGoods", Showoption.btnRecommend).unbind().on("click", function (e, data) {
                               ...
                               var index = layerCommon.openWindow('选择商品', "goodslist.aspx", '880px', '450px');
                       ...
                       BindFiveImgEvent: function () {
                           $("li .a-upload input:file", Showoption.btnFiveImg).unbind().on("change", function (e, data) {
                               var btnFile = this, str = $(btnFile).val(), Suffix = $.trim(str.substring(str.lastIndexOf("."))).toLowerCase(), $LI = $(btnFile).closest("li");
                               if (Suffix != ".gif" && Suffix != ".jpeg" && Suffix != ".jpg" && Suffix != ".png") {
                                   layerCommon.msg("请上传图片类型", IconOption.错误, 2500);
                                   return;
                               }
                               $.ajaxFileUpload({
                                   type: 'post',
                                   url: "/Handler/HandleImg3.ashx?FlileID=" + btnFile.id + "",
                                   secureuri: false,
                                   fileElementId: btnFile.id,
                                   dataType: 'text',
                                   success: function (msg, status) {
                           ...
                           $("li .website .SelectGoods", Showoption.btnFiveImg).unbind().on("click", function (e, data) {
                               ...
                               $Img.data("ImgUrl") == "" || $Img.data("ImgUrl") == undefined ? function () {
                                   ...
                                   var index = layerCommon.openWindow('选择商品', "goodslist.aspx?Type=Five", '880px', '450px');
                                   $("#hid_Alert").val(index); //记录弹出对象  
                               } ();
                           });
           ...
           var EditParam = {
               ...
               EditSetFiveImgGoodsValue: function (Id, Name) {
                   EditParam.EditSelectParentControl != null && function () {
                       $(".website .HidGoodsName", EditParam.EditSelectParentControl).val(Name), $(".website .SelectGoods", EditParam.EditSelectParentControl).text("重新选择商品地址"), $(".website .txt_GoodsUrl", EditParam.EditSelectParentControl).val(EditParam.WebDomainName + "/e" + Id + "_<%=CompID%>.html"), $(".website .HidGoodsID", EditParam.EditSelectParentControl).val(Id);
                       layerCommon.layerClose("hid_Alert");
                   } ();
               },
...
<%--		<div class="edit-btn"><a href="javascript:;" class="okbtn">确认装修</a></div>--%>
    ...
    <div class="edit_title"><div class="left bt"><i class="shops-i"></i>店铺装修</div><div class="right"><a href="/<%=CompID %>.html" target="_blank" class="okBtn">完成发布</a></div></div>
    <!--banner start-->

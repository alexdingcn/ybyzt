...
public partial class Distributor_Pay_orderPayList :DisPageBase
{
    ...
    public string Getmessage(int state, int id)
    {
        ...
        if (state == 0)
        {
            str = string.Format("<a href=\'javascript:void(0)\' onclick=\'pay(\"{0}\",\"{1}\")\' class=\"a-red\">立即支付</a>", Common.DesEncrypt(id.ToString(), Common.EncryptKey), Common.DesEncrypt(this.CompID.ToString(), Common.EncryptKey));

            //判断是否开启担保支付
            if (ConfigurationManager.AppSettings["IsDBPay"].ToString() == "1")
                strDB = string.Format("<a href=\'javascript:void(0)\' onclick=\'payDB(\"{0}\",\"{1}\")\'   class=\"a-red\">担保支付</a>", Common.DesEncrypt(id.ToString(), Common.EncryptKey), Common.DesEncrypt( this.CompID.ToString(), Common.EncryptKey));
        }
        else if (state == 1)
        {
            str = string.Format("<a href=\'javascript:void(0)\' onclick=\'pay(\"{0}\",\"{1}\")\'   class=\"a-red\">立即支付</a>", Common.DesEncrypt(id.ToString(), Common.EncryptKey), Common.DesEncrypt(this.CompID.ToString(), Common.EncryptKey));

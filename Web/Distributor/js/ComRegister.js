    ...
    $.extend({ BindRegister: function (BindType, CompId) {
        ...
        var CompRegister = {
            ...
            No2BindEevent: function (UserIsRegister) {
                var BindStr = UserIsRegister ? ".mianDiv .RegisterNo2  #txt_CompName" : ".mianDiv .RegisterNo2 input:text,.mianDiv .RegisterNo2 input:password";
            ...
            No2Submit: function (TControl, UserIsRegister) {
                ...
                CompRegister.AjaxCheck(TControl, CompRegister.DefaultUrl, { GetAction: "SubmitCheckNo2", Value: JSON.stringify(Data), RegisterType: BindType, Compid: CompId, FileValue: $.trim($("#HidFfileName").val()) }, function (ReturnData, Control) {
                    ...
                    if (ReturnData.Result) {
                        $(".mianDiv .RegisterNo2").fadeOut(800, function () {
                            (ReturnData.Href == "RegiDis" && $(".w1", ".mianDiv .RegisterNo3").html("加盟申请提交成功，请等待审核")) || (ReturnData.Href == "RegiOK" && $(".w1", ".mianDiv .RegisterNo3").html("加盟成功,请登录后操作"));
            ...
            , CheckNo1: function (CallBack) {
                ...
                if (!$("#CK_MYAgment").prop("checked")) {
                    $("#CK_MYAgment").parent().siblings("label").attr("class", "text tRed").text("请阅读并勾选接受my1818平台用户服务协议");
                    validator = false;
                }
                if (validator) {
                    CallBack();
            ...
            , CheckNo2: function (CallBack, UserIsRegister) {
                var validator = true;
                $.each($((UserIsRegister ? ".mianDiv .RegisterNo2  #txt_CompName" : ".mianDiv .RegisterNo2 input:text,.mianDiv .RegisterNo2 input:password")), function (index, Contorl) {
                    switch (this.id) {
                        case "txt_CompName":
                            if ($.trim($(this).val()) == "") {

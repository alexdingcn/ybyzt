...
$, win, ready = {
    ...
    shadeStyle = 'z-index:' + zIndex + '; background-color:' + config.shade[1] + '; opacity:' + config.shade[0] + '; filter:alpha(opacity=' + config.shade[0] * 100 + ');';
        ...
        config.border[3] && (border = '<div id="xubox_border' + (times+1) + '" class="xubox_border" style="' + borderStyle + '"></div>');
        config.closeBtn[1] && (closebtn = config.treeclose ? '' : '<a class="xubox_close xulayer_png32 xubox_close' + config.closeBtn[0] + '" href="javascript:;"></a>');
    ...
    Class.pt.creat = function () {
        ...
        this.layerB = $('#xubox_border' + (times+1));
        ...
        //        if (config.type == 0) {
        //            config.fix ? layerE.css({ top: parseInt(50) + config.border[0] }) : layerE.css({ top: parseInt(config.offset[0]) + win.scrollTop() + config.border[0], position: 'absolute' });
        //        } else 

        if (config.type == 2) {
            config.fix ? layerE.css({ top: parseInt(20) + config.border[0] }) : layerE.css({ top: parseInt(config.offset[0]) + win.scrollTop() + config.border[0] - 5, position: 'absolute' });
        } else {

...
public partial class Distributor_Rep_Tree_ProductSort : System.Web.UI.Page
{
    ...
    {
        var list = new Hi.BLL.BD_GoodsCategory().GetList("", "parentid=" + id + " and ISNULL(dr,0)=0 and compid=" + loguser.CompID, "");//判断是否存在下级类别
        return list.Count;
    }
    /// <summary>
    /// 商品类别
    ...
    {
        List<Hi.Model.BD_GoodsCategory> l = new Hi.BLL.BD_GoodsCategory().GetList("", " ISNULL(dr,0)=0 and compid=" + loguser.CompID, "sortindex, parentid");
        if (l.Count > 0)
        {

...
public partial class Distributor_ReceivingList : DisPageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            //自动签收
            //OrderInfoType.SendShip(user.ID, user.DisID, user.CompID);
        ...
        {
            string Id = Common.DesDecrypt(Server.UrlDecode(Request["orderid"].ToString()), Common.EncryptKey);
            int OID = Id.ToInt(0);
            btnSing_Click(OID);
        }
    }
    ...
    public void A_Seek(object sender, EventArgs e)
    {
        ...
        //}
        if (txtOderStartDate.Value.Trim() != "")
        {
            strwhere += " and orderid in(select id from dis_order where createDate >= '" + txtOderStartDate.Value.Trim() + "' and dr=0)";
        }
        if (txtOderEndDate.Value.Trim() != "")
        {
            strwhere += " and orderid in(select id from dis_order where createDate < '" + Convert.ToDateTime(txtOderEndDate.Value.Trim()).AddDays(1) + "' and dr=0)";
        }

<%@ WebHandler Language="C#" Class="CompImg" %>
...
public class CompImg : loginInfoMation,IHttpHandler
{
        ...
        if (context.Request.HttpMethod == "POST")
        {
            if (context.Request.Files.Count > 0)
            {
                try
                {
                    ...
                    string Fileype = context.Request["Fileype"] + "";
                    ...
                    {
                        Compid = Common.DesDecrypt((context.Request["CompKey"] + "").Trim(), Common.EncryptKey);
                    }

                    string path = ConfigurationManager.AppSettings["ImgPath"] + "CompImage" + "/";
                    if (!System.IO.Directory.Exists(path))
                    {
                        System.IO.Directory.CreateDirectory(path);
                    }
                    //文件名带后缀，根据时间产生唯一名称
                    //   string name = "temp" + ext;
                    string Sptor = "_";
                    if (Gettype == "SP" || Gettype == "SPNew")
                    {
                        Sptor += Gettype;
                    }
                    string name = DateTime.Now.ToString("yyyyMMdd-hhmmssffff") + (!string.IsNullOrWhiteSpace(Fileype) ? Fileype : "") + Sptor + Compid + ext;
                    string avatar_path = HandleHeadImg(path, name, b);
                    context.Response.Write(avatar_path);
                    return;
                }
                catch(Exception ex)
                {

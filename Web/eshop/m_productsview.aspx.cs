...
public partial class productsview : LoginPageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            List<Hi.Model.BD_ImageList> images = new Hi.BLL.BD_ImageList().GetList("", "isnull(dr,0)=0 and compid=" + compId + " and goodsId=" + goodsId, "");
            if (images.Count > 0)
            {
                DefaultProductImg.Visible = false;
                rptImg.DataSource = images;
                rptImg.DataBind();
            }
            else
            {
                DefaultProductImg.Visible = true;
                Hi.Model.BD_Goods model = new Hi.BLL.BD_Goods().GetModel(goodsId);
                if (model != null && !string.IsNullOrEmpty(model.Pic))
                {
                    this.defaultImg.Src = Common.GetPicURL(model.Pic, "resize400", logUser.CompID.ToString());
                    this.defaultImg.Attributes.Add("bimg", Common.GetPicURL(model.Pic, "", logUser.CompID.ToString()));
                }
            }
    ...
    public void Bind()
    {
        ...
        if (model != null)
        {
            ...
            {
                this.productDesc.InnerHtml = model.Details.Replace("<pre>", "<p>").Replace("</pre>", "</p>");//商品描述
            }
            if (!Util.IsEmpty(model.registeredCertificate.Trim()))
            {
                string url = "../../UploadFile/" + model.registeredCertificate;
                this.productCert.InnerHtml = "<img width=\"600\" src=\"" + url + "\"/>";
            }

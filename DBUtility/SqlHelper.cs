...
namespace DBUtility
{
    ...
    public abstract class SqlHelper
    {
        ...
        private static SqlConnection connections;
        //#endregion 
        /// <summary>
        /// 通用分页存储过程
        /// </summary>
        ...
        {
            SqlParameter[] parameters ={ new SqlParameter("@tblName",SqlDbType.NVarChar,1000),
        ...
        //通用存储过程2  （多表链接）
        public static DataSet PageList2(string connectionString, string tblName, string fldName, int pageSize, int pageIndex,
        string fldSort, bool Sort, string strCondition, string ID, bool Dist,
        out int pageCount, out int Counts, out string strSql, string CustomFldName = "")
        {
            SqlParameter[] parameters ={ new SqlParameter("@tblName",SqlDbType.NVarChar,4000),
                new SqlParameter("@fldName",SqlDbType.NVarChar,4000),
                new SqlParameter("@pageSize",SqlDbType.Int),
                new SqlParameter("@page",SqlDbType.Int),
                new SqlParameter("@fldSort",SqlDbType.NVarChar,1000),
                new SqlParameter("@Sort",SqlDbType.Bit),
                ...
                new SqlParameter("@Counts",SqlDbType.Int),
                new SqlParameter("@strSql",SqlDbType.NVarChar,4000),
                new SqlParameter("@CustomFldName",SqlDbType.NVarChar,4000),   
                                       };
            parameters[0].Value = tblName;
            parameters[1].Value = (fldName == null) ? "*" : fldName;
            parameters[2].Value = (pageSize == 0) ? int.Parse(ConfigurationManager.AppSettings["PageSize"]) : pageSize;
            ...
            parameters[10].Direction = ParameterDirection.Output;
            parameters[11].Direction = ParameterDirection.Output;
            parameters[12].Value = CustomFldName;
            DataSet ds = RunProcedure(connectionString, "PageList2", parameters, "ds");
        ...
        /// <summary>
        /// 执行带有事务的SQL语句，返回影响的记录数
        /// </summary>
        /// <param name="SQLString">SQL语句</param>
        /// <returns>影响的记录数</returns>
        public static int ExecuteSqlTran(string SQLString, SqlTransaction TranSction)
        {
            if (TranSction.Connection == null)
            {
                return 0;
            }
            using (SqlCommand cmd = new SqlCommand(SQLString, TranSction.Connection))
            {
                try
                {
        ...
        public static SqlTransaction CreateStoreTranSaction()
        {

            try
            {
                connections = new SqlConnection(SqlHelper.LocalSqlServer);
                connections.Open();
                return connections.BeginTransaction();
            }
            catch (Exception ex)
            {
                if (connections.State == ConnectionState.Broken || connections.State == ConnectionState.Open)
                    connections.Close();
                throw ex;
            }
        ...
        public static void ConnectionClose()
        {
            if (connections != null)
            {
                if (connections.State == ConnectionState.Open || connections.State == ConnectionState.Broken)
                {
                    connections.Close();
                }
            }
        ...
        /// <param name="SQLString">SQL语句</param>
        /// <returns>影响的记录数</returns>
        public static int ExecuteSql(string SQLString, SqlTransaction Transaction, params SqlParameter[] cmdParms)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                try
                {
                    PrepareCommand(cmd, Transaction.Connection, Transaction, SQLString, cmdParms);
                    int rows = cmd.ExecuteNonQuery();
        ...
        /// <summary>
        /// 执行查询语句，返回SqlDataReader 带有事物
        /// </summary>
        /// <param name="strSQL">查询语句</param>
        /// <returns>SqlDataReader</returns>
        public static SqlDataReader ExecuteReader(string strSQL, SqlTransaction Transaction)
        {
            SqlCommand cmd = new SqlCommand(strSQL, Transaction.Connection);
        ...
        /// <returns></returns>
        public static DataTable GetTable(string connectionString, string sql)
        {
        ...
        /// <param name="SQLString">查询语句</param>
        /// <returns>DataSet</returns>
        public static DataSet Query(string connectionString, string SQLString, SqlTransaction Tran)
        {
            if (SQLString != null && SQLString.Trim() != "")
            {
                ...
                try
                {
                    SqlDataAdapter command = new SqlDataAdapter(SQLString, Tran.Connection);
                    command.SelectCommand.Transaction = Tran;
                    command.Fill(ds, "ds");
                }
                catch (System.Data.SqlClient.SqlException ex)
                {
        ...
        /// <summary>
        /// (主表、附表专用)执行多条SQL语句，实现数据库事务。 如果传入的SQL的条数和执行后的结果数不相同、则事务直接回滚
        /// </summary>
        /// <param name="SQLStringList">多条SQL语句</param>		
        public static int ExecuteSqlTrans(string connectionString, List<string> SQLStringList)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                ...
                try
                {
                    ...
                    for (int n = 0; n < SQLStringList.Count; n++)
                    {
                        ...
                        if (strsql.Trim().Length > 1)
                        {
                            ...
                            //    else
                            //    {
                            //        maxStockInId = Convert.ToInt32(obj);
                            //    }
                            //    //int maxStockInId = new LSY.SQLServerDAL.A_StockIn().GetMaxId();//得到入库单最大id
                            //}
                            //else
                            //{
                            cmd.CommandText = strsql.Replace("values('0'", "values('" + maxStockInId.ToString() + "'");
        ...
        /// <summary>
        /// 执行一条带有事务的计算查询结果语句，返回查询结果（object）。
        /// </summary>
        /// <param name="SQLString">计算查询结果语句</param>
        /// <returns>查询结果（object）</returns>
        public static object GetSingle(string SQLString, SqlTransaction TranSction, params SqlParameter[] cmdParms)
        {
            if (TranSction.Connection == null)
            {
                return 0;
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                try
                {
                    PrepareCommand(cmd, TranSction.Connection, TranSction, SQLString, cmdParms);
                    object obj = cmd.ExecuteScalar();
                    cmd.Parameters.Clear();
                    if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
                    {
        ...
        public static SqlDataReader ExecuteReader(string SQLString, SqlTransaction TranSction, params SqlParameter[] cmdParms)
        {
            SqlCommand cmd = new SqlCommand();
            try
            {
        ...
        /// <param name="SQLString">查询语句</param>
        /// <returns>DataSet</returns>
        public static DataSet Query(string SQLString, SqlTransaction TranSction, params SqlParameter[] cmdParms)
        {
            SqlCommand cmd = new SqlCommand();
            PrepareCommand(cmd, TranSction.Connection, TranSction, SQLString, cmdParms);
            using (SqlDataAdapter da = new SqlDataAdapter(cmd))
        ...
        /// <returns></returns> 
        public static bool SqlBulkCopyInsert(string connectionString, DataTable dataTable, string table)
        {
            bool bol = false;
            SqlConnection sqlConnection = new SqlConnection(connectionString);
            sqlConnection.Open();
            SqlTransaction sqlbulkTransaction = sqlConnection.BeginTransaction();
            //请在插入数据的同时检查约束，如果发生错误调用sqlbulkTransaction事务
            SqlBulkCopy sqlBulkCopy = new SqlBulkCopy(sqlConnection, SqlBulkCopyOptions.CheckConstraints, sqlbulkTransaction);

            //设置要批量写入的表
            sqlBulkCopy.DestinationTableName = table;// "BD_GoodsPrice";
